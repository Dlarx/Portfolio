      ...
      >
        ...
        >
          <div>
            ...
            <h4 className="text-sm font-semibold text-[var(--text)] max-w-lg line-clamp-1">
              {cert.title}
            </h4>
          </div>
          <button
            onClick={onClose}
            className="w-8 h-8 rounded-lg glass flex items-center justify-center text-[var(--secondary)] hover:text-[var(--accent)] transition-colors ml-4 flex-shrink-0"
          >
            <X size={16} />
          </button>
        </div>
        ...
        {/* Certificate image */}
        <div className="bg-black/40 flex items-center justify-center p-4" style={{ minHeight: 360 }}>
          <motion.img
            src={cert.image}
              ...
              <AnimatePresence mode="wait">
                ...
                >
                  ...
                  <div className="flex items-center justify-between mt-2 pt-2 border-t border-white/10">
                    ...
                    {/* View Certificate button — opens lightbox */}
                    <button
                      onClick={() => setLightboxCert(CERTIFICATIONS[current])}
                      className="text-xs flex items-center gap-1.5 px-3 py-1.5 rounded-lg font-semibold text-white transition-all hover:opacity-90 hover:scale-105 active:scale-95"
                ...
                {CERTIFICATIONS.map((_, i) => (
                  <button
                    ...
                    className={`h-1.5 rounded-full transition-all duration-300 ${
                      i === current
                        ? "w-6 bg-[var(--accent)]"
                        : "w-1.5 bg-[var(--secondary)] opacity-30"
                    }`}
                  />
                ))}
              </div>
          ...
          <div className="flex flex-col gap-3">
            {CERTIFICATIONS.map((cert, i) => (
              <motion.button
                key={cert.title}
                onClick={() => setCurrent(i)}
                className={`text-left p-4 rounded-xl transition-all duration-300 flex items-center gap-4 ${
